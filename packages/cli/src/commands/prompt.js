...
function applyConfigToJsonc(data, config, { modelsOnly = false } = {}) {
  if (!modelsOnly) {
    ...
    for (const [agentName, agentConfig] of Object.entries(data.agents)) {
      if (!modelsOnly) {
        agentConfig.tools = {
          ...agentConfig.tools,
          ...config.agentTools,
          ...Object.fromEntries(config.deniedTools.map((tool) => [tool, false])),
        };
...
export async function promptCommand(targetPath, { modelsOnly = false } = {}) {
  const projectPath = path.resolve(targetPath || process.cwd());
  console.log(`Setting up OpenCode prompt config for: ${projectPath}${modelsOnly ? " (models only)" : ""}`);
  ...
  try {
    ...
    if (existsSync(agentJsoncPath)) {
      ...
      applyConfigToJsonc(data, config, { modelsOnly });
      writeFileSync(agentJsoncPath, JSON.stringify(data, null, 2));
      console.log(`Merged config.mjs → ${agentJsoncPath}${modelsOnly ? " (models only)" : ""}`);
